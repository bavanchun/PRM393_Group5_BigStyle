...
- [ ] `grep -rnE "toStringAsFixed\(0\)\}? ?đ" FE/lib` → 0 AND `grep -rn "NumberFormat" FE/lib --include="*.dart" | grep -v utils/currency_format.dart` → 0; all price displays render `10.000đ` style (emulator: home card, product detail, cart, checkout, orders, manager voucher list). Documented exemption: admin dashboard compact `tỷ`/`triệu` wrapper (grouping branch delegates to `formatVnd`). <!-- both greps re-verified 0 on 2026-07-12 + formatVnd unit tests green + exemption confirmed in code (admin_dashboard_screen.dart:291-299); the parenthetical emulator walk of the 6 screens was never documented — deferred to device pass (plans/260712-1644 Phase 1) -->
- [ ] All 6 FAB constructors in `FE/lib` carry a unique `heroTag`; admin login and manager voucher/category pushes produce no "multiple heroes" exception in `flutter run` log <!-- census re-verified 2026-07-12: 6 FAB constructor sites, all carrying heroTag (extra heroTag: hits today are PR #33 Hero-transition tags, not FABs); the post-fix flutter-run log check was never documented — deferred to device pass (plans/260712-1644 Phase 1) -->
- [x] Manager dashboard stat values render bold sans `textPrimary`, labels `textSecondary`; 4 cards have 4 distinct accents (primary / warning / info / accent) with pending = `warning` (matches `StatusBadge` mapping)
      <!-- code re-verified 2026-07-12 (manager_dashboard_widgets.dart: headlineLarge w700 value, 4 distinct accents, pending=warning) + widget test test/widgets/manager_stats_grid_test.dart green -->
- [x] OTP: paste (incl. noisy clipboard → standalone 6-digit run) fills and auto-submits; backspace on empty box moves back; editing a middle box then completing re-submits; re-entering the identical code after clear re-submits; boxes + resend + Google + debug buttons disabled with "Đang xác thực..." spinner in the resend slot during verify; boxes cleared ONLY on verify-originated errors
      <!-- behaviors pinned by test/screens/auth/otp_input_test.dart (7 widget tests incl. noisy-paste regression) per journal 260710-2141 -->
- [x] Resend link disabled with "Gửi lại sau {n}s" countdown for 60s per email (dispatch-started, single-rate tick, per-email bypass on address change); resend path goes through the validator; email validator rejects `a@`, `@b`, accepts `hoangbavan4478+admin@gmail.com`; verify targets the `AuthOTPSent` email, not live field text
      <!-- validator matrix unit-tested (test/utils/validators_test.dart); cooldown mechanics + _otpEmail targeting verified by the code-review audit recorded in journal 260710-2141 -->
- [x] `flutter analyze` 0 issues, `flutter test` green (43 existing + new), hardcode-guard still 0, at every phase boundary <!-- per journal (every boundary); re-verified 2026-07-12: analyze 0, 116/116 tests, guard exit 0 -->
