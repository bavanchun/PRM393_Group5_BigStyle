...
**Live-tap verification (resolved 2026-07-10, before Phase 3):** user manually tapped the cart CTA on `emulator-5554` with an item selected — works fine by hand. Confirms the capture-tool-artifact hypothesis; not a real bug. Cart (Phase 3) proceeds with token-only migration, no bug-fix step/commit. Checkout/PaymentQr still get migrated from code + Phase 1 metrics alone regardless of this verdict (no customer-role QA credentials available this session to live-capture them either way) — flagged `unverified` in Phase 3's completion note per the plan's own fallback path.

**Manager FAB verification (resolved 2026-07-10, before Phase 5's ManagerProductList/Detail/Create):** user manually tapped the ManagerProductList FAB on `emulator-5554` — also works fine by hand. Same verdict: capture-tool artifact, not a real bug. ManagerProductList/Detail proceed with token-only migration, no bug-fix step. ManagerCreateProduct still gets migrated from code alone (no manager-role QA credentials this session to live-capture it), flagged `unverified` in Phase 5's completion note.
